# Мой личный справочник по ООП

#### За основу справочника взята книга "PHP 8 объекты, шаблоны и методики программирования, 6-е издание [2021] Мэтт Зандстра". Дополнено собственными заметками.
### Основы (азы объектов)
- [Основные понятия](basics/classes-and-objects.md)
- [Методы-конструкторы](basics/methods-constructors.md)
- [Примитивные типы и типы класса](basics/basic-types-and-classes.md)
- [Наследование](basics/inheritance.md)
- [Видимость](basics/visibility.md)

### Расширеннные возможности
- [Статические методы и свойства](more_features/static.md)
- [Абстрактные классы и интерфейсы](more_features/classes.md)
- [Трейты](more_features/classes.md)
- [Обработка ошибок](more_features/classes.md)
- [Завершенные классы и методы](more_features/classes.md)
- [Методы-перехватчики](more_features/classes.md)
- [Методы-деструкторы](more_features/classes.md)
- [Клонирование объектов](more_features/classes.md)
- [Преобразование объектов в символьные строки](more_features/classes.md)
- [Функции обратного вызова](more_features/classes.md)
